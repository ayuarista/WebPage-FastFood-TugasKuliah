...
function getkat() {
    global $con; 
    $get_kategori = "SELECT * FROM produk";
    $run_kategori = mysqli_query($con, $get_kategori);
    
    while ($row_kategori=mysqli_fetch_array($run_kategori)){ 
        $kode_kategori=$row_kategori['kode_produk'];
        $nama_kategori=$row_kategori['nama_produk'];  
    echo "<li><a href='detail.php?produk=$kode_kategori'>$nama_kategori</a></li>" ;   
    }
}
...
function getcatpro() {
    ...
    if(isset($_GET['produk'])){
            $kode_kategori =$_GET['produk'];

        global $con;

        $get_cat_pro = "SELECT * FROM produk WHERE kode_produk='$kode_kategori' ";
        ...
        while ($row_cat_pro=mysqli_fetch_array($run_cat_pro)) {
            ...
            <p><b> Rasa Produk: $rasa_pro </b </p>
            <p> <b> Status Produk: $status_pro </b></p>
            </div>
            </div>
            ";
        }
          ...
          <div class="col-md-2"> 
            <div class="sidebar sidebar-fixed-center" role="menubar">    
              <nav class="navbar navbar-inverse navbar-fixed-right" role="navigation">
                <div class="container">
                <div class="navbar-header">
                <a class="navbar-brand" href="menu.php"><font size="6" color="#c0c0c0">Produk</font></a>
                <div class="collapse navbar-collapse">
                </div>     
                </div><!--.nav-collapse -->
                </div>
